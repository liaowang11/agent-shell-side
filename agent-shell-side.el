...
(defun agent-shell-side-buffer-p (&optional buffer)
  ...
  (and (agent-shell-side--live-buffer
...
(defun agent-shell-side--status-label (status)
  ...
  (pcase status
    ('needs-approval "main needs approval")
...
(defun agent-shell-side--unwatch-parent (side-buffer)
  ...
  (when-let* ((token (buffer-local-value 'agent-shell-side--parent-subscription
                                         side-buffer))
...
asked.  Close it with `agent-shell-side-dismiss'.
...
(defun agent-shell-side-toggle ()
  ...
  (let ((target (or (agent-shell-side--live-buffer agent-shell-side--parent-buffer)
    ...
    (when-let* ((window (get-buffer-window target)))
...
`agent-shell-side-resume'."
  (interactive)
  (let ((side-buffer (if (agent-shell-side-buffer-p)
                         (current-buffer)
